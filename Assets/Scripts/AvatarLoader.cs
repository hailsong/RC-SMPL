...
{
    public TrackerLoader KinectDevice;
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < (int)JointId.Count; i++)
        {
            ...
            if (hbb != HumanBodyBones.LastBone)
            {
                ...
                // Debug.Log($"Avatar Loader.cs Start() / {absOffset.eulerAngles}");
            }
        }
    }
    ...
    public void AnimationUpdate(Quaternion[] jointRotations)
    {
        for (int j = 0; j < (int)JointId.Count; j++)
        {
            if (MapKinectJoint((JointId)j) != HumanBodyBones.LastBone && absoluteOffsetMap.ContainsKey((JointId)j))
            {
                ...
                finalJoint.rotation = absOffset * Quaternion.Inverse(absOffset) * jointRotations[j] * absOffset;
                if (j == 0)
                {
