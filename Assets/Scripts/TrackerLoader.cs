...
public class TrackerLoader : MonoBehaviour
{
    ...
    public void updateLoadedTracker(Body loadedbody)
    {
        renderSkeleton(loadedbody, 0);
    }
    ...
    public void renderSkeleton(Body skeleton, int skeletonNumber)
    {
        for (int jointNum = 0; jointNum < (int)JointId.Count; jointNum++)
        {
            ...
            //Debug.Log($"TrackerLoader, {jointRot}");
            //Debug.Log($"absoluteJointRotationsr, {absoluteJointRotations[jointNum]}");
    ...
    public Quaternion[] returnJointRotationsSkeleton(Body skeleton)
    {
